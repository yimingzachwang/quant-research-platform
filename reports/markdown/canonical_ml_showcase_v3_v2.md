# Experiment Report: canonical_ml_showcase_v3_v2
...
![Cross Asset Volatility](../figures/canonical_ml_showcase_v3_v2/cross_asset_volatility.png)
...
![Rolling Volatility](../figures/canonical_ml_showcase_v3_v2/rolling_volatility.png)
...
![Feature Correlation Heatmap](../figures/canonical_ml_showcase_v3_v2/feature_correlation_heatmap.png)
...
![Ml Feature Regimes](../figures/canonical_ml_showcase_v3_v2/ml_feature_regimes.png)
...
![Feature Ic Heatmap](../figures/canonical_ml_showcase_v3_v2/feature_ic_heatmap.png)
...
![Feature Family Ic](../figures/canonical_ml_showcase_v3_v2/feature_family_ic.png)
...
![Portfolio Turnover](../figures/canonical_ml_showcase_v3_v2/portfolio_turnover.png)
...
| Mean monthly IC (Pearson) | 0.4975 |
| % months with positive IC | 87.8% |
| Aligned training samples | 2,747 |

![Ml Ic Regime](../figures/canonical_ml_showcase_v3_v2/ml_ic_regime.png)

*Mean IC 0.498 (88% positive) — strong directional signal. Sub-period troughs mark regimes where the feature space lost predictive content.*

![Ml Rolling Da](../figures/canonical_ml_showcase_v3_v2/ml_rolling_da.png)
...
![Ml Residuals](../figures/canonical_ml_showcase_v3_v2/ml_residuals.png)

*Symmetric residuals near zero indicate an unbiased model. Skew or heavy tails signal calibration failure in extreme regimes. Rolling residual mean reveals temporal bias drift. 92% positive predictions — long-biased; errors skew toward missed downside. Directional accuracy: 66.4%.*

**Prediction confidence calibration:**

| Quintile | N | Mean pred | Mean actual | Dir. accuracy |
| --- | --- | --- | --- | --- |
| Q1 | 550 | +0.0001 | +0.0059 | 59.3% |
| Q2 | 549 | +0.0044 | +0.0033 | 63.8% |
| Q3 | 549 | +0.0080 | +0.0046 | 65.4% |
| Q4 | 549 | +0.0125 | +0.0140 | 75.0% |
| Q5 | 550 | +0.0257 | +0.0229 | 68.7% |

*Q5 accuracy (68.7%) exceeds Q1 (59.3%) — prediction magnitude carries information; higher-conviction signals are more accurate.*

**Feature importance summary** (sorted by sign consistency):

| Feature | Full-period coef | Mean OOS coef | Sign consistency |
| --- | --- | --- | --- |
| 60D Momentum | -0.0798 | -0.0641 | 86% |
| 252D Momentum | +0.0199 | -0.0388 | 71% |
| 21D Realized Volatility | +0.0647 | +0.0357 | 71% |
| 20D Z-Score | -0.0002 | -0.0003 | 71% |
| 20D Momentum | -0.0414 | -0.0102 | 57% |
| 20D Trend Strength | -0.0016 | -0.0024 | 57% |
| 5D Momentum | -0.0082 | -0.0047 | 43% |
...
![Ml Coefficient Stability](../figures/canonical_ml_showcase_v3_v2/ml_coefficient_stability.png)

*60D Momentum most stable (86% sign consistency); 5D Momentum least stable (43%) — regime-dependent contribution. Wide error bars or sign reversals indicate training-regime-specific fitting.*

![Ml Coefficient Sign Heatmap](../figures/canonical_ml_showcase_v3_v2/ml_coefficient_sign_heatmap.png)
...
| Diagnostic | Value |
| --- | --- |
| Dominant family (by contribution share) | Trend (81% of periods) |
| Family leadership transitions | 19 |
| Mean contribution concentration (HHI) | 0.567 — concentrated (one family dominates) |
| Most temporally volatile feature | 60D Momentum |

The **Trend** family dominated predictions in 81% of periods, indicating the model's directional prediction signal was concentrated in a single predictive hypothesis for most of the backtest. This is characteristic of a momentum-regime environment where trend information dominates.

Contribution structure was stable and concentrated — 19 family leadership transitions across the backtest — consistent with a model that operated in persistent predictive regimes rather than frequently adapting.


![Feature Contribution Heatmap](../figures/canonical_ml_showcase_v3_v2/feature_contribution_heatmap.png)
...
![Family Contribution Timeline](../figures/canonical_ml_showcase_v3_v2/family_contribution_timeline.png)
...
![Ic By Vol Regime](../figures/canonical_ml_showcase_v3_v2/ic_by_vol_regime.png)
...
![Equity And Drawdown](../figures/canonical_ml_showcase_v3_v2/equity_and_drawdown.png)

*Max drawdown -34.1% — severe. Sharpe 0.67. Slow recovery indicates signal-edge loss, not random noise.*

![Rolling Sharpe](../figures/canonical_ml_showcase_v3_v2/rolling_sharpe.png)
...
![Walk Forward Stitched](../figures/canonical_ml_showcase_v3_v2/walk_forward_stitched.png)
...
![Walk Forward Timeline](../figures/canonical_ml_showcase_v3_v2/walk_forward_timeline.png)
...
![Split Sharpes](../figures/canonical_ml_showcase_v3_v2/split_sharpes.png)
...
![Train Vs Test Sharpe](../figures/canonical_ml_showcase_v3_v2/train_vs_test_sharpe.png)

*Mean OOS Sharpe -0.26 across 7 splits (negative, 43% positive). Large per-split gaps mark regime-specific overfitting.*

## Failure Analysis

**Failure analysis — ML signal:**

*Out-of-sample verdict:* RidgeRegression failed to generalise out-of-sample: 4 of 7 test windows produced negative Sharpe (mean OOS Sharpe: -0.26). The in-sample signal does not survive chronological validation — the primary evidence of overfitting or regime specificity.

**Known failure modes for ML price-history strategies:**

- *Regime non-stationarity* — Feature-return relationships shift across macro regimes (risk-on/risk-off, trending/mean-reverting, high/low volatility). A model trained on 2013–2016 data learns relationships that may not hold in 2022 tightening cycles or 2020 dislocation episodes. Mean IC of 0.498 confirms directional signal exists in aggregate, but sub-period IC variation (visible in the IC chart) reveals regime dependence.
...
| --- | --- | --- | --- | --- |
| Low | 909 | 9.86 | 0.0600 | 73.2% |
| Medium | 909 | 4.68 | 0.1821 | 65.7% |
| High | 909 | 1.65 | 0.3733 | 61.2% |
...
![Split Equity Curves](../figures/canonical_ml_showcase_v3_v2/split_equity_curves.png)
...
| Mean annualised return | -3.83% |
| Mean max drawdown | -19.80% |
...
![Ml Signal Turnover](../figures/canonical_ml_showcase_v3_v2/ml_signal_turnover.png)
...
| Field | Value |
| --- | --- |
| Experiment | `canonical_ml_showcase_v3_v2` |
| Strategy | `MLStrategy(Ridge(alpha=1.0))` |
| Created | 2026-06-06T12:57:39.231814+00:00 |
...
![Allocation History](../figures/canonical_ml_showcase_v3_v2/allocation_history.png)
...
![Ml Information Coefficient](../figures/canonical_ml_showcase_v3_v2/ml_information_coefficient.png)
...
![Ml Prediction Vs Actual](../figures/canonical_ml_showcase_v3_v2/ml_prediction_vs_actual.png)
...
![Ml Prediction Distribution](../figures/canonical_ml_showcase_v3_v2/ml_prediction_distribution.png)
...
![Ml Coefficient Evolution](../figures/canonical_ml_showcase_v3_v2/ml_coefficient_evolution.png)
...
| Key | Value |
| --- | --- |
| ML hash | `4962c24e7a34` |

---

Report version: 1
Generated: 2026-06-06T12:57:48.095783+00:00
Source experiment: canonical_ml_showcase_v3_v2