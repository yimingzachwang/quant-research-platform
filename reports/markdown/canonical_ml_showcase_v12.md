# Experiment Report: canonical_ml_showcase_v12
...
**Hypothesis:** The feature space constructed from SPY price history contains statistically reliable information about 21-day forward returns. A RidgeRegression model trained on 8 engineered features can extract this signal and translate it into a risk-adjusted return edge after transaction costs.
...
![Cross Asset Volatility](../figures/canonical_ml_showcase_v12/cross_asset_volatility.png)
...
![Rolling Volatility](../figures/canonical_ml_showcase_v12/rolling_volatility.png)
...
**Feature registry** — 8 features constructed on `SPY`:
...
| Risk Adjusted Momentum 20 | Trend | momentum_over_vol | — | raw |
...
| 60D Momentum | 0.0300 | 0.0628 | -0.70 | 0.969 | 98.0% |
...
| Risk Adjusted Momentum 20 | 0.0949 | 0.2523 | -0.23 | 0.935 | 97.9% |

![Feature Correlation Heatmap](../figures/canonical_ml_showcase_v12/feature_correlation_heatmap.png)
...
![Ml Feature Regimes](../figures/canonical_ml_showcase_v12/ml_feature_regimes.png)
...
![Feature Ic Heatmap](../figures/canonical_ml_showcase_v12/feature_ic_heatmap.png)
...
| --- | --- | --- |
| Trend | 20D Momentum, 60D Momentum, 252D Momentum, 20D Trend Strength, 5D Momentum, Risk Adjusted Momentum 20 | Directional momentum and price-continuation signals. Positive IC in trending regimes; may … |
...
This experiment deploys 8 features across 3 canonical families. Each family operationalises a distinct market hypothesis; orthogonal families provide diversified information sources, reducing model dependence on any single regime hypothesis.

![Feature Family Ic](../figures/canonical_ml_showcase_v12/feature_family_ic.png)
...
```
prices[t-window : t]
  → feature_matrix('mom_20', 'mom_60', 'mom_252', 'vol_21', ... (8 total))
       (X: 8 columns, no NaN rows, pre-alignment)
...
![Portfolio Turnover](../figures/canonical_ml_showcase_v12/portfolio_turnover.png)
...
| Metric | Value |
| --- | --- |
| Directional accuracy (full period) | 65.9% |
| Mean monthly IC (Pearson) | 0.4944 |
| % months with positive IC | 86.3% |
| Aligned training samples | 2,747 |

![Ml Ic Regime](../figures/canonical_ml_showcase_v12/ml_ic_regime.png)

*Mean IC 0.494 (86% positive) — strong directional signal. Sub-period troughs mark regimes where the feature space lost predictive content.*

![Ml Rolling Da](../figures/canonical_ml_showcase_v12/ml_rolling_da.png)

*Full-period directional accuracy 65.9%. Periods below 50% indicate the model generated net-incorrect directional calls — regime-linked degradation, not statistical noise.*


![Ml Residuals](../figures/canonical_ml_showcase_v12/ml_residuals.png)

*Symmetric residuals near zero indicate an unbiased model. Skew or heavy tails signal calibration failure in extreme regimes. Rolling residual mean reveals temporal bias drift. 92% positive predictions — long-biased; errors skew toward missed downside. Directional accuracy: 65.9%.*

**Prediction confidence calibration:**

| Quintile | N | Mean pred | Mean actual | Dir. accuracy |
| --- | --- | --- | --- | --- |
| Q1 | 550 | +0.0000 | +0.0052 | 55.3% |
| Q2 | 549 | +0.0045 | +0.0040 | 65.4% |
| Q3 | 549 | +0.0080 | +0.0043 | 64.5% |
| Q4 | 549 | +0.0124 | +0.0147 | 75.8% |
| Q5 | 550 | +0.0258 | +0.0226 | 68.5% |

*Q5 accuracy (68.5%) exceeds Q1 (55.3%) — prediction magnitude carries information; higher-conviction signals are more accurate.*

**Feature importance summary** (sorted by sign consistency):

| Feature | Full-period coef | Mean OOS coef | Sign consistency |
| --- | --- | --- | --- |
| 60D Momentum | -0.0845 | -0.0816 | 86% |
| 20D Trend Strength | -0.0020 | -0.0024 | 86% |
| 252D Momentum | +0.0205 | -0.0445 | 57% |
| 21D Realized Volatility | +0.0656 | +0.0284 | 57% |
| 20D Z-Score | -0.0003 | -0.0001 | 57% |
| 20D Momentum | -0.0580 | -0.0116 | 43% |
| 5D Momentum | -0.0096 | -0.0064 | 43% |
| Risk Adjusted Momentum 20 | +0.0049 | +0.0008 | 43% |
...
![Ml Coefficient Stability](../figures/canonical_ml_showcase_v12/ml_coefficient_stability.png)

*60D Momentum most stable (86% sign consistency); 20D Momentum least stable (43%) — regime-dependent contribution. Wide error bars or sign reversals indicate training-regime-specific fitting.*

![Ml Coefficient Sign Heatmap](../figures/canonical_ml_showcase_v12/ml_coefficient_sign_heatmap.png)

*`20D Momentum`, `252D Momentum`, `21D Realized Volatility` change sign across splits — regime-specific learning, not stable directional relationships. Stable features maintain colour throughout; reversals are the instability signature.*
...
| Diagnostic | Value |
| --- | --- |
| Dominant family (by contribution share) | Trend (85% of periods) |
| Family leadership transitions | 22 |
| Mean contribution concentration (HHI) | 0.578 — concentrated (one family dominates) |
| Most temporally volatile feature | 60D Momentum |

The **Trend** family dominated predictions in 85% of periods, indicating the model's directional prediction signal was concentrated in a single predictive hypothesis for most of the backtest. This is characteristic of a momentum-regime environment where trend information dominates.

Contribution structure was stable and concentrated — 22 family leadership transitions across the backtest — consistent with a model that operated in persistent predictive regimes rather than frequently adapting.


![Feature Contribution Heatmap](../figures/canonical_ml_showcase_v12/feature_contribution_heatmap.png)
...
![Family Contribution Timeline](../figures/canonical_ml_showcase_v12/family_contribution_timeline.png)
...
![Ic By Vol Regime](../figures/canonical_ml_showcase_v12/ic_by_vol_regime.png)
...
| Annualized Volatility | 0.1646 |
| Sharpe Ratio | 0.7041 |
| Max Drawdown | -0.3410 |
| Calmar Ratio | 0.3158 |
| Hit Rate | 0.4901 |

![Equity And Drawdown](../figures/canonical_ml_showcase_v12/equity_and_drawdown.png)

*Max drawdown -34.1% — severe. Sharpe 0.70. Slow recovery indicates signal-edge loss, not random noise.*

![Rolling Sharpe](../figures/canonical_ml_showcase_v12/rolling_sharpe.png)
...
![Walk Forward Stitched](../figures/canonical_ml_showcase_v12/walk_forward_stitched.png)
...
![Walk Forward Timeline](../figures/canonical_ml_showcase_v12/walk_forward_timeline.png)
...
![Split Sharpes](../figures/canonical_ml_showcase_v12/split_sharpes.png)
...
![Train Vs Test Sharpe](../figures/canonical_ml_showcase_v12/train_vs_test_sharpe.png)

*Mean OOS Sharpe -0.53 across 7 splits (negative, 14% positive). Large per-split gaps mark regime-specific overfitting.*

## Failure Analysis

**Failure analysis — ML signal:**

*Out-of-sample verdict:* RidgeRegression failed to generalise out-of-sample: 6 of 7 test windows produced negative Sharpe (mean OOS Sharpe: -0.53). The in-sample signal does not survive chronological validation — the primary evidence of overfitting or regime specificity.

**Known failure modes for ML price-history strategies:**

- *Regime non-stationarity* — Feature-return relationships shift across macro regimes (risk-on/risk-off, trending/mean-reverting, high/low volatility). A model trained on 2013–2016 data learns relationships that may not hold in 2022 tightening cycles or 2020 dislocation episodes. Mean IC of 0.494 confirms directional signal exists in aggregate, but sub-period IC variation (visible in the IC chart) reveals regime dependence.
- *Feature instability* — `mom_20`, `mom_252`, `vol_21`, `zscore_20` show sign consistency below 60% across walk-forward splits, indicating their directional contribution reverses in different training regimes. The model is learning regime-specific patterns, not persistent market relationships.
...
| --- | --- | --- | --- | --- |
| Low | 909 | 9.86 | 0.0550 | 73.2% |
| Medium | 909 | 4.68 | 0.1674 | 64.0% |
| High | 909 | 1.65 | 0.3736 | 61.2% |
...
Coefficient sign reversals in 20D Momentum, 252D Momentum, 21D Realized Volatility indicate the model learned regime-specific rather than structural relationships. IC is consistently positive (86% of periods, mean 0.4944), propagating coefficient instability into signal degradation. Directional accuracy (65.9%) is above random; signal degradation from unstable features reaches realised trade outcomes. The Sharpe compression in high-volatility regimes confirms the propagation completes: feature instability → IC breakdown → ranking errors → losses.

**Identified drawdown windows (> 5% peak-to-trough):**

| Drawdown Start | Trough | Recovery | Max DD | Duration |
| --- | --- | --- | --- | --- |
| 2020-02-25 | 2020-03-23 | 2020-06-11 | -34.1% | 107d |
| 2022-04-05 | 2022-06-17 | 2023-12-01 | -23.4% | 605d |
| 2018-12-04 | 2018-12-24 | 2019-02-21 | -20.2% | 79d |

**Worst out-of-sample split:**

- Split 6 — test period 2023-01-03 to 2023-12-29
- Sharpe: -1.73
- Return: -21.1%
- Max DD: -21.8%

![Split Equity Curves](../figures/canonical_ml_showcase_v12/split_equity_curves.png)
...
| Mean Sharpe | -0.5271 |
| Std Sharpe | 1.1078 |
| Positive-Sharpe rate | 14.3% |
| Mean annualised return | -9.14% |
| Mean max drawdown | -20.80% |
| Worst max drawdown | -39.28% |

### ML Signal Diagnostics

| Metric | Value |
| --- | --- |
| Avg daily turnover | 0.0533 |
| Signal activity | 91.6% |

![Ml Signal Turnover](../figures/canonical_ml_showcase_v12/ml_signal_turnover.png)
...
| Field | Value |
| --- | --- |
| Experiment | `canonical_ml_showcase_v12` |
| Strategy | `MLStrategy(Ridge(alpha=0.5))` |
| Created | 2026-06-07T14:04:04.337537+00:00 |
...
![Allocation History](../figures/canonical_ml_showcase_v12/allocation_history.png)
...
![Ml Information Coefficient](../figures/canonical_ml_showcase_v12/ml_information_coefficient.png)
...
![Ml Prediction Vs Actual](../figures/canonical_ml_showcase_v12/ml_prediction_vs_actual.png)
...
![Ml Prediction Distribution](../figures/canonical_ml_showcase_v12/ml_prediction_distribution.png)
...
![Ml Coefficient Evolution](../figures/canonical_ml_showcase_v12/ml_coefficient_evolution.png)
...
| Key | Value |
| --- | --- |
| ML hash | `a029931f54e7` |

---

Report version: 1
Generated: 2026-06-07T14:04:13.168665+00:00
Source experiment: canonical_ml_showcase_v12