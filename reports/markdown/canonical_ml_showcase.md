# Experiment Report: canonical_ml_showcase
...
**Hypothesis:** The feature space constructed from SPY price history contains statistically reliable information about 21-day forward returns. A RidgeRegression model trained on 7 engineered features can extract this signal and translate it into a risk-adjusted return edge after transaction costs.
...
![Cross Asset Volatility](../figures/canonical_ml_showcase/cross_asset_volatility.png)
...
![Rolling Volatility](../figures/canonical_ml_showcase/rolling_volatility.png)
...
**Feature registry** — 7 features constructed on `SPY`:
...
![Feature Correlation Heatmap](../figures/canonical_ml_showcase/feature_correlation_heatmap.png)
...
![Ml Feature Regimes](../figures/canonical_ml_showcase/ml_feature_regimes.png)
...
![Feature Ic Heatmap](../figures/canonical_ml_showcase/feature_ic_heatmap.png)
...
| --- | --- | --- |
| Trend | 20D Momentum, 60D Momentum, 252D Momentum, 20D Trend Strength, 5D Momentum | Directional momentum and price-continuation signals. Positive IC in trending regimes; may … |
...
This experiment deploys 7 features across 3 canonical families. Each family operationalises a distinct market hypothesis; orthogonal families provide diversified information sources, reducing model dependence on any single regime hypothesis.

![Feature Family Ic](../figures/canonical_ml_showcase/feature_family_ic.png)
...
```
prices[t-window : t]
  → feature_matrix('mom_20', 'mom_60', 'mom_252', 'vol_21', ... (7 total))
       (X: 7 columns, no NaN rows, pre-alignment)
...
![Portfolio Turnover](../figures/canonical_ml_showcase/portfolio_turnover.png)
...
| Metric | Value |
| --- | --- |
| Directional accuracy (full period) | 66.4% |
| Mean monthly IC (Pearson) | 0.4877 |
| % months with positive IC | 87.0% |
| Aligned training samples | 2,747 |

![Ml Ic Regime](../figures/canonical_ml_showcase/ml_ic_regime.png)

*Mean IC 0.488 (87% positive) — strong directional signal. Sub-period troughs mark regimes where the feature space lost predictive content.*

![Ml Rolling Da](../figures/canonical_ml_showcase/ml_rolling_da.png)

*Full-period directional accuracy 66.4%. Periods below 50% indicate the model generated net-incorrect directional calls — regime-linked degradation, not statistical noise.*


![Ml Residuals](../figures/canonical_ml_showcase/ml_residuals.png)

*Symmetric residuals near zero indicate an unbiased model. Skew or heavy tails signal calibration failure in extreme regimes. Rolling residual mean reveals temporal bias drift. 91% positive predictions — long-biased; errors skew toward missed downside. Directional accuracy: 66.4%.*

**Prediction confidence calibration:**

| Quintile | N | Mean pred | Mean actual | Dir. accuracy |
| --- | --- | --- | --- | --- |
| Q1 | 550 | -0.0001 | +0.0057 | 58.9% |
| Q2 | 549 | +0.0044 | +0.0030 | 64.1% |
| Q3 | 549 | +0.0080 | +0.0045 | 64.7% |
| Q4 | 549 | +0.0125 | +0.0148 | 75.6% |
| Q5 | 550 | +0.0260 | +0.0227 | 68.7% |

*Q5 accuracy (68.7%) exceeds Q1 (58.9%) — prediction magnitude carries information; higher-conviction signals are more accurate.*

**Feature importance summary** (sorted by sign consistency):

| Feature | Full-period coef | Mean OOS coef | Sign consistency |
| --- | --- | --- | --- |
| 60D Momentum | -0.0841 | -0.0814 | 86% |
| 20D Trend Strength | -0.0012 | -0.0022 | 71% |
| 252D Momentum | +0.0209 | -0.0436 | 57% |
| 21D Realized Volatility | +0.0648 | +0.0283 | 57% |
| 20D Z-Score | -0.0001 | -0.0000 | 57% |
| 5D Momentum | -0.0092 | -0.0065 | 57% |
| 20D Momentum | -0.0481 | -0.0116 | 43% |
...
![Ml Coefficient Stability](../figures/canonical_ml_showcase/ml_coefficient_stability.png)
...
![Ml Coefficient Sign Heatmap](../figures/canonical_ml_showcase/ml_coefficient_sign_heatmap.png)
...
| Diagnostic | Value |
| --- | --- |
| Dominant family (by contribution share) | Trend (83% of periods) |
| Family leadership transitions | 17 |
| Mean contribution concentration (HHI) | 0.577 — concentrated (one family dominates) |
| Most temporally volatile feature | 60D Momentum |

The **Trend** family dominated predictions in 83% of periods, indicating the model's directional prediction signal was concentrated in a single predictive hypothesis for most of the backtest. This is characteristic of a momentum-regime environment where trend information dominates.

Contribution structure was stable and concentrated — 17 family leadership transitions across the backtest — consistent with a model that operated in persistent predictive regimes rather than frequently adapting.


![Feature Contribution Heatmap](../figures/canonical_ml_showcase/feature_contribution_heatmap.png)
...
![Family Contribution Timeline](../figures/canonical_ml_showcase/family_contribution_timeline.png)
...
![Ic By Vol Regime](../figures/canonical_ml_showcase/ic_by_vol_regime.png)
...
![Equity And Drawdown](../figures/canonical_ml_showcase/equity_and_drawdown.png)

*Max drawdown -34.1% — severe. Sharpe 0.69. Slow recovery indicates signal-edge loss, not random noise.*

![Rolling Sharpe](../figures/canonical_ml_showcase/rolling_sharpe.png)
...
![Walk Forward Stitched](../figures/canonical_ml_showcase/walk_forward_stitched.png)
...
![Walk Forward Timeline](../figures/canonical_ml_showcase/walk_forward_timeline.png)
...
![Split Sharpes](../figures/canonical_ml_showcase/split_sharpes.png)
...
![Train Vs Test Sharpe](../figures/canonical_ml_showcase/train_vs_test_sharpe.png)

*Mean OOS Sharpe -0.32 across 7 splits (negative, 43% positive). Large per-split gaps mark regime-specific overfitting.*

## Failure Analysis

**Failure analysis — ML signal:**

*Out-of-sample verdict:* RidgeRegression failed to generalise out-of-sample: 4 of 7 test windows produced negative Sharpe (mean OOS Sharpe: -0.32). The in-sample signal does not survive chronological validation — the primary evidence of overfitting or regime specificity.

**Known failure modes for ML price-history strategies:**

- *Regime non-stationarity* — Feature-return relationships shift across macro regimes (risk-on/risk-off, trending/mean-reverting, high/low volatility). A model trained on 2013–2016 data learns relationships that may not hold in 2022 tightening cycles or 2020 dislocation episodes. Mean IC of 0.488 confirms directional signal exists in aggregate, but sub-period IC variation (visible in the IC chart) reveals regime dependence.
...
| --- | --- | --- | --- | --- |
| Low | 909 | 9.86 | 0.0542 | 73.3% |
| Medium | 909 | 4.68 | 0.1753 | 65.3% |
| High | 909 | 1.65 | 0.3732 | 61.3% |
...
Coefficient sign reversals in 20D Momentum, 252D Momentum, 21D Realized Volatility indicate the model learned regime-specific rather than structural relationships. IC is consistently positive (87% of periods, mean 0.4877), propagating coefficient instability into signal degradation. Directional accuracy (66.4%) is above random; signal degradation from unstable features reaches realised trade outcomes. The Sharpe compression in high-volatility regimes confirms the propagation completes: feature instability → IC breakdown → ranking errors → losses.
...
| 2022-04-05 | 2022-10-12 | 2023-12-13 | -25.2% | 617d |
...
![Split Equity Curves](../figures/canonical_ml_showcase/split_equity_curves.png)
...
| Mean Sharpe | -0.3200 |
| Std Sharpe | 1.2741 |
| Positive-Sharpe rate | 42.9% |
| Mean annualised return | -4.08% |
| Mean max drawdown | -19.67% |
| Worst max drawdown | -30.92% |

### ML Signal Diagnostics

| Metric | Value |
| --- | --- |
| Avg daily turnover | 0.0586 |
| Signal activity | 91.6% |

![Ml Signal Turnover](../figures/canonical_ml_showcase/ml_signal_turnover.png)
...
![Allocation History](../figures/canonical_ml_showcase/allocation_history.png)
...
![Ml Information Coefficient](../figures/canonical_ml_showcase/ml_information_coefficient.png)
...
![Ml Prediction Vs Actual](../figures/canonical_ml_showcase/ml_prediction_vs_actual.png)
...
![Ml Prediction Distribution](../figures/canonical_ml_showcase/ml_prediction_distribution.png)
...
![Ml Coefficient Evolution](../figures/canonical_ml_showcase/ml_coefficient_evolution.png)
