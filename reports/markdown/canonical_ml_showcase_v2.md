# Experiment Report: canonical_ml_showcase_v2
...
![Cross Asset Volatility](../figures/canonical_ml_showcase_v2/cross_asset_volatility.png)
...
![Rolling Volatility](../figures/canonical_ml_showcase_v2/rolling_volatility.png)
...
![Feature Correlation Heatmap](../figures/canonical_ml_showcase_v2/feature_correlation_heatmap.png)
...
![Ml Feature Regimes](../figures/canonical_ml_showcase_v2/ml_feature_regimes.png)
...
![Feature Ic Heatmap](../figures/canonical_ml_showcase_v2/feature_ic_heatmap.png)
...
![Feature Family Ic](../figures/canonical_ml_showcase_v2/feature_family_ic.png)
...
![Portfolio Turnover](../figures/canonical_ml_showcase_v2/portfolio_turnover.png)
...
| Mean monthly IC (Pearson) | 0.5005 |
| % months with positive IC | 88.5% |
| Aligned training samples | 2,747 |

![Ml Ic Regime](../figures/canonical_ml_showcase_v2/ml_ic_regime.png)

*Mean IC 0.500 (89% positive) — strong directional signal. Sub-period troughs mark regimes where the feature space lost predictive content.*

![Ml Rolling Da](../figures/canonical_ml_showcase_v2/ml_rolling_da.png)
...
![Ml Residuals](../figures/canonical_ml_showcase_v2/ml_residuals.png)

*Symmetric residuals near zero indicate an unbiased model. Skew or heavy tails signal calibration failure in extreme regimes. Rolling residual mean reveals temporal bias drift. 96% positive predictions — long-biased; errors skew toward missed downside. Directional accuracy: 66.8%.*

**Prediction confidence calibration:**

| Quintile | N | Mean pred | Mean actual | Dir. accuracy |
| --- | --- | --- | --- | --- |
| Q1 | 550 | +0.0013 | +0.0018 | 56.9% |
| Q2 | 549 | +0.0045 | +0.0100 | 67.9% |
| Q3 | 549 | +0.0080 | +0.0036 | 67.9% |
| Q4 | 549 | +0.0127 | +0.0131 | 71.8% |
| Q5 | 550 | +0.0243 | +0.0222 | 69.6% |

*Q5 accuracy (69.6%) exceeds Q1 (56.9%) — prediction magnitude carries information; higher-conviction signals are more accurate.*

**Feature importance summary** (sorted by sign consistency):

| Feature | Full-period coef | Mean OOS coef | Sign consistency |
| --- | --- | --- | --- |
| 252D Momentum | +0.0117 | -0.0500 | 71% |
| 21D Realized Volatility | +0.0868 | +0.0530 | 71% |
| 20D Momentum | -0.0644 | -0.0181 | 57% |
| 20D Z-Score | -0.0006 | -0.0006 | 57% |
| 20D Trend Strength | -0.0016 | -0.0023 | 57% |
| 5D Momentum | -0.0145 | -0.0079 | 57% |
...
![Ml Coefficient Stability](../figures/canonical_ml_showcase_v2/ml_coefficient_stability.png)
...
![Ml Coefficient Sign Heatmap](../figures/canonical_ml_showcase_v2/ml_coefficient_sign_heatmap.png)

*`20D Momentum`, `20D Z-Score`, `20D Trend Strength` change sign across splits — regime-specific learning, not stable directional relationships. Stable features maintain colour throughout; reversals are the instability signature.*
...
| Diagnostic | Value |
| --- | --- |
| Dominant family (by contribution share) | Trend (51% of periods) |
| Family leadership transitions | 47 |
| Mean contribution concentration (HHI) | 0.526 — concentrated (one family dominates) |
| Most temporally volatile feature | 21D Realized Volatility |

The **Trend** family led predictions in 51% of periods, but shared dominance with other families across the sample. Multiple hypothesis families contributed meaningful predictive information, suggesting regime-conditioned feature activation.

Contribution structure was stable and concentrated — 47 family leadership transitions across the backtest — consistent with a model that operated in persistent predictive regimes rather than frequently adapting.


![Feature Contribution Heatmap](../figures/canonical_ml_showcase_v2/feature_contribution_heatmap.png)
...
![Family Contribution Timeline](../figures/canonical_ml_showcase_v2/family_contribution_timeline.png)
...
![Ic By Vol Regime](../figures/canonical_ml_showcase_v2/ic_by_vol_regime.png)
...
![Equity And Drawdown](../figures/canonical_ml_showcase_v2/equity_and_drawdown.png)

*Max drawdown -34.1% — severe. Sharpe 0.63. Slow recovery indicates signal-edge loss, not random noise.*

![Rolling Sharpe](../figures/canonical_ml_showcase_v2/rolling_sharpe.png)
...
![Walk Forward Stitched](../figures/canonical_ml_showcase_v2/walk_forward_stitched.png)
...
![Walk Forward Timeline](../figures/canonical_ml_showcase_v2/walk_forward_timeline.png)
...
![Split Sharpes](../figures/canonical_ml_showcase_v2/split_sharpes.png)
...
![Train Vs Test Sharpe](../figures/canonical_ml_showcase_v2/train_vs_test_sharpe.png)

*Mean OOS Sharpe -0.40 across 7 splits (negative, 29% positive). Large per-split gaps mark regime-specific overfitting.*

## Failure Analysis

**Failure analysis — ML signal:**

*Out-of-sample verdict:* RidgeRegression failed to generalise out-of-sample: 5 of 7 test windows produced negative Sharpe (mean OOS Sharpe: -0.40). The in-sample signal does not survive chronological validation — the primary evidence of overfitting or regime specificity.

**Known failure modes for ML price-history strategies:**

- *Regime non-stationarity* — Feature-return relationships shift across macro regimes (risk-on/risk-off, trending/mean-reverting, high/low volatility). A model trained on 2013–2016 data learns relationships that may not hold in 2022 tightening cycles or 2020 dislocation episodes. Mean IC of 0.500 confirms directional signal exists in aggregate, but sub-period IC variation (visible in the IC chart) reveals regime dependence.
- *Feature instability* — `mom_20`, `zscore_20`, `trend_20`, `mom_5` show sign consistency below 60% across walk-forward splits, indicating their directional contribution reverses in different training regimes. The model is learning regime-specific patterns, not persistent market relationships.
...
| --- | --- | --- | --- | --- |
| Low | 909 | 9.86 | 0.1489 | 74.5% |
| Medium | 909 | 4.68 | 0.2629 | 66.7% |
| High | 909 | 1.65 | 0.3179 | 60.1% |
...
Coefficient sign reversals in 20D Momentum, 20D Z-Score, 20D Trend Strength indicate the model learned regime-specific rather than structural relationships. IC is consistently positive (89% of periods, mean 0.5005), propagating coefficient instability into signal degradation. Directional accuracy (66.8%) is above random; signal degradation from unstable features reaches realised trade outcomes. The Sharpe compression in high-volatility regimes confirms the propagation completes: feature instability → IC breakdown → ranking errors → losses.
...
| 2022-04-05 | 2022-10-12 | 2023-07-28 | -25.4% | 479d |
...
![Split Equity Curves](../figures/canonical_ml_showcase_v2/split_equity_curves.png)
...
| Mean annualised return | -4.85% |
| Mean max drawdown | -19.45% |
...
| Metric | Value |
| --- | --- |
| Avg daily turnover | 0.0401 |
| Signal activity | 91.6% |

![Ml Signal Turnover](../figures/canonical_ml_showcase_v2/ml_signal_turnover.png)
...
| Field | Value |
| --- | --- |
| Experiment | `canonical_ml_showcase_v2` |
| Strategy | `MLStrategy(Ridge(alpha=0.5))` |
| Created | 2026-06-05T21:41:57.290469+00:00 |
...
![Allocation History](../figures/canonical_ml_showcase_v2/allocation_history.png)
...
![Ml Information Coefficient](../figures/canonical_ml_showcase_v2/ml_information_coefficient.png)
...
![Ml Prediction Vs Actual](../figures/canonical_ml_showcase_v2/ml_prediction_vs_actual.png)
...
![Ml Prediction Distribution](../figures/canonical_ml_showcase_v2/ml_prediction_distribution.png)
...
![Ml Coefficient Evolution](../figures/canonical_ml_showcase_v2/ml_coefficient_evolution.png)
...
| Key | Value |
| --- | --- |
| ML hash | `ab0d4e17210f` |

---

Report version: 1
Generated: 2026-06-05T21:42:05.987718+00:00
Source experiment: canonical_ml_showcase_v2