# Experiment Report: canonical_ml_showcase_v11
...
![Cross Asset Volatility](../figures/canonical_ml_showcase_v11/cross_asset_volatility.png)
...
![Rolling Volatility](../figures/canonical_ml_showcase_v11/rolling_volatility.png)
...
![Feature Correlation Heatmap](../figures/canonical_ml_showcase_v11/feature_correlation_heatmap.png)
...
![Ml Feature Regimes](../figures/canonical_ml_showcase_v11/ml_feature_regimes.png)
...
![Feature Ic Heatmap](../figures/canonical_ml_showcase_v11/feature_ic_heatmap.png)
...
![Feature Family Ic](../figures/canonical_ml_showcase_v11/feature_family_ic.png)
...
![Portfolio Turnover](../figures/canonical_ml_showcase_v11/portfolio_turnover.png)
...
| Metric | Value |
| --- | --- |
| Directional accuracy (full period) | 67.0% |
| Mean monthly IC (Pearson) | 0.5120 |
| % months with positive IC | 88.5% |
| Aligned training samples | 2,747 |

![Ml Ic Regime](../figures/canonical_ml_showcase_v11/ml_ic_regime.png)

*Mean IC 0.512 (89% positive) — strong directional signal. Sub-period troughs mark regimes where the feature space lost predictive content.*

![Ml Rolling Da](../figures/canonical_ml_showcase_v11/ml_rolling_da.png)

*Full-period directional accuracy 67.0%. Periods below 50% indicate the model generated net-incorrect directional calls — regime-linked degradation, not statistical noise.*


![Ml Residuals](../figures/canonical_ml_showcase_v11/ml_residuals.png)

*Symmetric residuals near zero indicate an unbiased model. Skew or heavy tails signal calibration failure in extreme regimes. Rolling residual mean reveals temporal bias drift. 93% positive predictions — long-biased; errors skew toward missed downside. Directional accuracy: 67.0%.*

**Prediction confidence calibration:**

| Quintile | N | Mean pred | Mean actual | Dir. accuracy |
| --- | --- | --- | --- | --- |
| Q1 | 550 | +0.0004 | +0.0053 | 61.8% |
| Q2 | 549 | +0.0045 | +0.0034 | 63.8% |
| Q3 | 549 | +0.0081 | +0.0052 | 65.2% |
| Q4 | 549 | +0.0126 | +0.0143 | 76.0% |
| Q5 | 550 | +0.0252 | +0.0225 | 68.2% |

*Q5 accuracy (68.2%) exceeds Q1 (61.8%) — prediction magnitude carries information; higher-conviction signals are more accurate.*

**Feature importance summary** (sorted by sign consistency):

| Feature | Full-period coef | Mean OOS coef | Sign consistency |
| --- | --- | --- | --- |
| 60D Momentum | -0.0726 | -0.0481 | 100% |
| 252D Momentum | +0.0178 | -0.0328 | 71% |
| 21D Realized Volatility | +0.0638 | +0.0396 | 71% |
| 20D Z-Score | -0.0003 | -0.0005 | 71% |
| 20D Momentum | -0.0330 | -0.0082 | 57% |
| 20D Trend Strength | -0.0021 | -0.0028 | 57% |
| 5D Momentum | -0.0066 | -0.0029 | 43% |
...
![Ml Coefficient Stability](../figures/canonical_ml_showcase_v11/ml_coefficient_stability.png)

*60D Momentum most stable (100% sign consistency); 5D Momentum least stable (43%) — regime-dependent contribution. Wide error bars or sign reversals indicate training-regime-specific fitting.*

![Ml Coefficient Sign Heatmap](../figures/canonical_ml_showcase_v11/ml_coefficient_sign_heatmap.png)
...
| Diagnostic | Value |
| --- | --- |
| Dominant family (by contribution share) | Trend (77% of periods) |
| Family leadership transitions | 29 |
| Mean contribution concentration (HHI) | 0.556 — concentrated (one family dominates) |
| Most temporally volatile feature | 60D Momentum |

The **Trend** family dominated predictions in 77% of periods, indicating the model's directional prediction signal was concentrated in a single predictive hypothesis for most of the backtest. This is characteristic of a momentum-regime environment where trend information dominates.

Contribution structure was stable and concentrated — 29 family leadership transitions across the backtest — consistent with a model that operated in persistent predictive regimes rather than frequently adapting.


![Feature Contribution Heatmap](../figures/canonical_ml_showcase_v11/feature_contribution_heatmap.png)
...
![Family Contribution Timeline](../figures/canonical_ml_showcase_v11/family_contribution_timeline.png)
...
![Ic By Vol Regime](../figures/canonical_ml_showcase_v11/ic_by_vol_regime.png)
...
![Equity And Drawdown](../figures/canonical_ml_showcase_v11/equity_and_drawdown.png)

*Max drawdown -34.1% — severe. Sharpe 0.68. Slow recovery indicates signal-edge loss, not random noise.*

![Rolling Sharpe](../figures/canonical_ml_showcase_v11/rolling_sharpe.png)
...
![Walk Forward Stitched](../figures/canonical_ml_showcase_v11/walk_forward_stitched.png)
...
![Walk Forward Timeline](../figures/canonical_ml_showcase_v11/walk_forward_timeline.png)
...
![Split Sharpes](../figures/canonical_ml_showcase_v11/split_sharpes.png)
...
![Train Vs Test Sharpe](../figures/canonical_ml_showcase_v11/train_vs_test_sharpe.png)

*Mean OOS Sharpe -0.25 across 7 splits (negative, 29% positive). Large per-split gaps mark regime-specific overfitting.*

## Failure Analysis

**Failure analysis — ML signal:**

*Out-of-sample verdict:* RidgeRegression failed to generalise out-of-sample: 5 of 7 test windows produced negative Sharpe (mean OOS Sharpe: -0.25). The in-sample signal does not survive chronological validation — the primary evidence of overfitting or regime specificity.

**Known failure modes for ML price-history strategies:**

- *Regime non-stationarity* — Feature-return relationships shift across macro regimes (risk-on/risk-off, trending/mean-reverting, high/low volatility). A model trained on 2013–2016 data learns relationships that may not hold in 2022 tightening cycles or 2020 dislocation episodes. Mean IC of 0.512 confirms directional signal exists in aggregate, but sub-period IC variation (visible in the IC chart) reveals regime dependence.
...
| --- | --- | --- | --- | --- |
| Low | 909 | 9.86 | 0.0680 | 73.8% |
| Medium | 909 | 4.68 | 0.1924 | 66.7% |
| High | 909 | 1.65 | 0.3731 | 61.2% |
...
Coefficient sign reversals in 20D Momentum, 20D Trend Strength, 5D Momentum indicate the model learned regime-specific rather than structural relationships. IC is consistently positive (89% of periods, mean 0.5120), propagating coefficient instability into signal degradation. Directional accuracy (67.0%) is above random; signal degradation from unstable features reaches realised trade outcomes. The Sharpe compression in high-volatility regimes confirms the propagation completes: feature instability → IC breakdown → ranking errors → losses.
...
| 2022-04-05 | 2022-10-12 | 2023-08-02 | -25.2% | 484d |
...
![Split Equity Curves](../figures/canonical_ml_showcase_v11/split_equity_curves.png)
...
| Mean annualised return | -1.09% |
| Mean max drawdown | -19.92% |
| Worst max drawdown | -34.10% |
...
![Ml Signal Turnover](../figures/canonical_ml_showcase_v11/ml_signal_turnover.png)
...
| Field | Value |
| --- | --- |
| Experiment | `canonical_ml_showcase_v11` |
| Strategy | `MLStrategy(Ridge(alpha=2))` |
| Created | 2026-06-06T21:46:42.632998+00:00 |
...
![Allocation History](../figures/canonical_ml_showcase_v11/allocation_history.png)
...
![Ml Information Coefficient](../figures/canonical_ml_showcase_v11/ml_information_coefficient.png)
...
![Ml Prediction Vs Actual](../figures/canonical_ml_showcase_v11/ml_prediction_vs_actual.png)
...
![Ml Prediction Distribution](../figures/canonical_ml_showcase_v11/ml_prediction_distribution.png)
...
![Ml Coefficient Evolution](../figures/canonical_ml_showcase_v11/ml_coefficient_evolution.png)
...
| Key | Value |
| --- | --- |
| ML hash | `2fd214fa8dab` |

---

Report version: 1
Generated: 2026-06-06T21:46:51.139244+00:00
Source experiment: canonical_ml_showcase_v11